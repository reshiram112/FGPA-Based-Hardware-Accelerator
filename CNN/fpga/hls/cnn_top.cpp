...
void cnn_top(
    ...
    #pragma HLS INTERFACE m_axi port=m_axi_w_conv1 depth=72 offset=slave bundle=weights
    #pragma HLS INTERFACE m_axi port=m_axi_b_conv1 depth=8 offset=slave bundle=weights
    #pragma HLS INTERFACE m_axi port=m_axi_w_conv2 depth=1152 offset=slave bundle=weights
    #pragma HLS INTERFACE m_axi port=m_axi_b_conv2 depth=16 offset=slave bundle=weights
    #pragma HLS INTERFACE m_axi port=m_axi_w_fc depth=7840 offset=slave bundle=weights
    #pragma HLS INTERFACE m_axi port=m_axi_b_fc depth=10 offset=slave bundle=weights
    ...
    // Weight Buffers
    data_t w_conv1[CONV1_CH][IMG_CH][CONV1_K][CONV1_K];
    data_t b_conv1[CONV1_CH];
    data_t w_conv2[CONV2_CH][CONV1_CH][CONV2_K][CONV2_K];
    data_t b_conv2[CONV2_CH];
    data_t w_fc[FC_OUT][FC_IN];
    data_t b_fc[FC_OUT];
    ...
    // --------------------------------------------------------
    // Load Weights from DDR (Burst Mode)
    // --------------------------------------------------------
    LOAD_W_CONV1: for(int i=0; i<CONV1_CH*IMG_CH*CONV1_K*CONV1_K; i++) {
        #pragma HLS PIPELINE
        ((data_t*)w_conv1)[i] = m_axi_w_conv1[i];
    }
    LOAD_B_CONV1: for(int i=0; i<CONV1_CH; i++) {
        #pragma HLS PIPELINE
        b_conv1[i] = m_axi_b_conv1[i];
    }
    LOAD_W_CONV2: for(int i=0; i<CONV2_CH*CONV1_CH*CONV2_K*CONV2_K; i++) {
        #pragma HLS PIPELINE
        ((data_t*)w_conv2)[i] = m_axi_w_conv2[i];
    }
    LOAD_B_CONV2: for(int i=0; i<CONV2_CH; i++) {
        #pragma HLS PIPELINE
        b_conv2[i] = m_axi_b_conv2[i];
    }
    LOAD_W_FC: for(int i=0; i<FC_OUT*FC_IN; i++) {
        #pragma HLS PIPELINE
        ((data_t*)w_fc)[i] = m_axi_w_fc[i];
    }
    LOAD_B_FC: for(int i=0; i<FC_OUT; i++) {
        #pragma HLS PIPELINE
        b_fc[i] = m_axi_b_fc[i];
    }
                ...
                CONV1_K_R: for(int kr=-1; kr<=1; kr++) {
                    CONV1_K_C: for(int kc=-1; kc<=1; kc++) {
                        ...
                        if(r_idx >= 0 && r_idx < IMG_H && c_idx >= 0 && c_idx < IMG_W) {
                            sum += img_buf[0][r_idx][c_idx] * w_conv1[ch_out][0][kr+1][kc+1];
                        }
                    }
                }
                // ReLU
                    ...
                    CONV2_K_R: for(int kr=-1; kr<=1; kr++) {
                        CONV2_K_C: for(int kc=-1; kc<=1; kc++) {
                            ...
                            if(r_idx >= 0 && r_idx < P1_H && c_idx >= 0 && c_idx < P1_W) {
                                sum += pool1_buf[ch_in][r_idx][c_idx] * w_conv2[ch_out][ch_in][kr+1][kc+1];
                            }
                        }
                    }
                }
        ...
        FC_IN_CH: for(int ch=0; ch<CONV2_CH; ch++) {
            FC_IN_R: for(int r=0; r<P2_H; r++) {
                FC_IN_C: for(int c=0; c<P2_W; c++) {
                    #pragma HLS PIPELINE
                    sum += pool2_buf[ch][r][c] * w_fc[o][in_idx];
                    in_idx++;
                }
            }
        }
    ...
    WRITE_OUT: for(int i=0; i<FC_OUT; i++) {
        ...
        pkt.data(15,0) = raw;
        pkt.data(31,16) = 0; // Pad to 32-bit DMA width
