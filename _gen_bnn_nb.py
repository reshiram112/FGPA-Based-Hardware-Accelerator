"""Generate bnn_inference_pynq.ipynb for CAN vehicle BNN."""
import json, pathlib, uuid

NB = pathlib.Path(
    r"C:\Users\TA17084\fpga_radioml_mlp\Akshay-v1-work\bnn_accelerator\fpga\pynq\bnn_inference_pynq.ipynb"
)
...
cells.append(md_cell(
    "# CAN Vehicle Classifier — BNN FPGA Inference (PYNQ-Z2)\n\n"
    "**Model:** Bayesian MLP `169 → 16 (ReLU) → 3`  (mean weights only)\n"
    "**Task:** CAN bus vehicle classification — OpelAstra / Prototype / RenaultClio\n"
    "**Board:** PYNQ-Z2 (Zynq XC7Z020)  \n\n"
    "---\n\n"
    "| File | Description |\n"
    "|------|-------------|\n"
    "| `bnn_accelerator.bit` | Vivado bitstream |\n"
    "| `bnn_accelerator.hwh` | Hardware handoff |\n"
    "| `bnn_can_feature_stats.npz` | Feature normalisation stats |\n\n"
    "**Stream protocol (free-running):**\n"
    "- **Send:** 169 × uint32 words — one Q8.8 int16 feature per word (676 bytes)\n"
    "- **Recv:** 1 × uint32 word — argmax class 0/1/2\n"
    "- `dma.sendchannel.transfer(in_buf)` → `dma.recvchannel.transfer(out_buf)` → `wait()` × 2"
))
...
# ── Cell 2: Load Feature Stats + Bitstream ───────────────────────────────
cells.append(code_cell(
    "# ============================================================\n"
...
# ── Cell 4: Feature Extraction & Inference ───────────────────────────────
cells.append(code_cell(
    "# ============================================================\n"
...
# ── Cell 6: Interactive Upload (.log file) ────────────────────────────────
cells.append(code_cell(
    "# ============================================================\n"
...
# ── Also write .py version ────────────────────────────────────────────────
PY = NB.with_suffix(".py")
py_lines = ["#!/usr/bin/env python3", "# bnn_inference_pynq.py -- CAN Vehicle BNN inference on PYNQ-Z2", ""]
for cell in cells:
    if cell["cell_type"] == "code":
        py_lines.append(cell["source"])
        py_lines.append("")
PY.write_text("\n".join(py_lines), encoding="ascii")
print(f"Written: {PY}  ({PY.stat().st_size} bytes)")
