...
#  Target : bnn inference accelerator with PS+PL
#
#  Weight loading: DMA MM2S → bnn s_axis_input (stream, mode=1)
#  Inference:      DMA MM2S → bnn → DMA S2MM   (stream, mode=0)
# ============================================================

# ── 0. Settings ──────────────────────────────────────────────
set project_name  "mnist_bnn"
...
# ── 5. Add bnn HLS IP ─────────────────────────────────────────
set bnn [create_bd_cell -type ip -vlnv xilinx.com:hls:bnn_top:1.0 bnn_top_0]

# ── 6. Control AXI Interconnect: PS GP0 → DMA + bnn ctrl ─────
...
connect_bd_net [get_bd_pins rst_gen/peripheral_aresetn] \
    [get_bd_pins bnn_top_0/ap_rst_n] \
...
# ── 9. AXI-Lite control: PS → DMA + bnn ctrl ────────────────
...
connect_bd_intf_net [get_bd_intf_pins axi_ic_ctrl/M01_AXI] \
    [get_bd_intf_pins bnn_top_0/s_axi_ctrl]

# ── 10. AXI-Stream: DMA → bnn → DMA ─────────────────────────
connect_bd_intf_net [get_bd_intf_pins axi_dma_0/M_AXIS_MM2S] \
    [get_bd_intf_pins bnn_top_0/s_axis_input]
connect_bd_intf_net [get_bd_intf_pins bnn_top_0/m_axis_output] \
...
puts "\n=== bnn BUILD COMPLETE ==="
puts "Bitstream : ${out_dir}/bnn.bit"
puts "XSA file  : ${out_dir}/bnn.xsa"
