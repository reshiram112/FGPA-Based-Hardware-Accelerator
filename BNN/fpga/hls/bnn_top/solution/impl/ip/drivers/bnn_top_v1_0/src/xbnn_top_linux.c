...
typedef struct {
    ...
    XBnn_top_uio_map maps[ MAX_UIO_MAPS ];
} XBnn_top_uio_info;

/***************** Variable Definitions **************************************/
static XBnn_top_uio_info uio_info;
...
static int uio_info_read_name(XBnn_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XBnn_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XBnn_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XBnn_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XBnn_top_Initialize(XBnn_top *InstancePtr, const char* InstanceName) {
	XBnn_top_uio_info *InfoPtr = &uio_info;
