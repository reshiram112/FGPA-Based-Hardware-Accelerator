...
FC_FILE  = str(BASE_DIR / "models" / "cnn" / "cnn_fc_weights.npz")
...
if not Path(FC_FILE).exists():
    print(f"ERROR: FC weights not found: {FC_FILE}")
    sys.exit(1)

overlay = Overlay(BIT_FILE)
dma     = overlay.axi_dma_0
fc_data = np.load(FC_FILE)
...
def preprocess(jpeg_bytes: bytes) -> np.ndarray:
    """Decode image, normalize like training, and convert to CHW Q8.8 uint32."""
...
def cnn_predict(q8_img: np.ndarray):
    """Run co-design inference and return (class_id, hw_ms, total_ms)."""
...
while True:
    ...
    try:
        while True:
            ...
            q8   = preprocess(jpeg)
            pred, hw_lat, total_lat = cnn_predict(q8)
            ...
            frame_count += 1
            print(f"  [{frame_count:5d}] {label:<40}  total {total_lat:.2f} ms  hw {hw_lat:.2f} ms")
