...
setlocal
REM ============================================================
REM build_and_deploy.bat -- Siren CNN FPGA full pipeline
REM
...
REM   Step 3 : Vivado bitstream     -> cnn_vivado_files\cnn_accelerator.bit
REM   Step 4 : Extract .hwh         -> cnn_vivado_files\cnn.hwh
REM   Step 5 : SCP deploy to board  -> 192.168.2.99
REM
REM Usage (run from ANY directory -- paths are relative to this bat):
REM   CNN\fpga\build_and_deploy.bat
REM
...
set "WEIGHTS_H=%HLS_DIR%\cnn_weights_data.h"
set "HLS_ZIP=%FPGA%hls_ip.zip"
set "HLS_IP=%FPGA%hls_ip"

echo.
echo ============================================================
echo  Siren CNN FPGA Accelerator -- Full Build Pipeline
echo  Python: %PYTHON%
echo ============================================================
...
if exist "%WEIGHTS_H%" (
    echo       cnn_weights_data.h already exists -- skipping.
...
echo [3/5] Vivado done. Bitstream: %PYNQ_DIR%\cnn_vivado_files\cnn_accelerator.bit
...
echo [4/5] Done. HWH: %PYNQ_DIR%\cnn_vivado_files\cnn.hwh
...
echo  Navigate to  cnn_accelerator/
echo  Open         cnn_inference_pynq.ipynb
