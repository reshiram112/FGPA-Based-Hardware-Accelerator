# create_project.tcl -- Vivado block design + bitstream for Siren MLP
#
# Generates: mlp_vivado_files/mlp_accelerator.bit + mlp_accelerator.hwh
...
#   IN  : 41 uint32 words (Q4.12 features) = 164 B
#   OUT : 1   uint32 word  (argmax class idx) =    4 B
#   Weights are baked into BRAM -- not sent over DMA.

set project_name  "mlp_accelerator"
set project_dir   "[file normalize [file dirname [info script]]]/proj_${project_name}"
set bd_name       "mlp_accelerator"                     ;# -> mlp_accelerator.bit / .hwh
set part          "xc7z020clg400-1"
set hls_ip_vlnv   "xilinx.com:hls:mlp_top:1.0"

# ---------------- Log file: writes to stdout AND vivado_build.log simultaneously ------------------------------------------------------------------------------------------------
set LOG_PATH "[file normalize [file dirname [info script]]]/vivado_build.log"
...
log "  DMA OUT : 1  uint32 word  (argmax: 0=ambulance, 1=firetruck) = 4 B"
log "  Log     : ${LOG_PATH}"
log "======================================================"
...
#   fpga/hls_ip/xilinx.com_hls_mlp_top_1.0/
...
#   Required: 256x4=1024 B (in) + 4 B (out)  well within 64 KB
...
set hls_ip [create_bd_cell -type ip -vlnv ${hls_ip_vlnv} mlp_top_0]
...
#   DMA.M_AXIS_MM2S -------- mlp_top_0.s_axis_input   (send: host -------- kernel)
#   mlp_top_0.m_axis_output -------- DMA.S_AXIS_S2MM  (recv: kernel -------- host)
connect_bd_intf_net [get_bd_intf_pins axi_dma_0/M_AXIS_MM2S]    [get_bd_intf_pins mlp_top_0/s_axis_input]
connect_bd_intf_net [get_bd_intf_pins mlp_top_0/m_axis_output]  [get_bd_intf_pins axi_dma_0/S_AXIS_S2MM]
...
# -- Output to mlp_vivado_files/ (mirrors bnn_vivado_files/ etc. in reference) --
set vf_dir "[file normalize [file dirname [info script]]]/../pynq/mlp_vivado_files"
file mkdir ${vf_dir}
file copy -force $bit_src "${vf_dir}/mlp_accelerator.bit"
log "Bitstream copied: ${vf_dir}/mlp_accelerator.bit  ([expr {[file size ${vf_dir}/mlp_accelerator.bit] / 1024 / 1024}] MB)"

write_hw_platform -fixed -force -include_bit "${out_dir}/mlp_accelerator.xsa"
log "XSA written    : ${out_dir}/mlp_accelerator.xsa"

log "======================================================"
log "Vivado build COMPLETE"
log "  Bitstream : ${vf_dir}/mlp_accelerator.bit"
log "  XSA       : ${out_dir}/mlp_accelerator.xsa"
log "  Log       : ${LOG_PATH}"
log ""
log "Next steps:"
log "  1. Extract .hwh from XSA:"
log "       python fpga/pynq/extract_hwh.py --xsa ${out_dir}/mlp_accelerator.xsa \\"
log "           --out-dir fpga/pynq/mlp_vivado_files"
log "  2. .bit already in mlp_vivado_files/ -- deploy to PYNQ:"
log "       python fpga/pynq/deploy.py"
log "  3. On the board, open Jupyter and run: radioml_mlp/mlp_inference_pynq.py"
log "      (or: inference.ipynb if preferred)"
log "======================================================"
