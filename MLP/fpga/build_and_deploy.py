?#!/usr/bin/env python3
"""
build_and_deploy.py - Complete MLP FPGA accelerator build + deploy pipeline.
...
Run from workspace root  C:\\Users\\TA17084\\fpga_siren_mlp\\ :

  "C:\\Program Files\\Python313\\python.exe" MLP\\fpga\\build_and_deploy.py
...
# ?????? Paths ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
HERE   = pathlib.Path(__file__).resolve().parent   # MLP/fpga/
HLS    = HERE / 'hls'
VIVADO = HERE / 'vivado'
PYNQ   = HERE / 'pynq'
VF     = PYNQ / 'mlp_vivado_files'
...
def step0(auto_yes):
    ...
    out = HLS / 'mlp_weights_data.h'
    if out.exists():
        print(f'  Found: mlp_weights_data.h  ({out.stat().st_size // 1024} KB)')
        if not confirm_overwrite('mlp_weights_data.h', auto_yes):
...
def step3(auto_yes):
    ...
    out = VF / 'mlp_accelerator.bit'
    if out.exists():
        mb = out.stat().st_size // (1024 * 1024)
        print(f'  Found: mlp_accelerator.bit  ({mb} MB)')
        if not confirm_overwrite('mlp_accelerator.bit', auto_yes):
...
def step4():
    ...
    xsa = VIVADO / 'proj_mlp_accelerator' / 'output' / 'mlp_accelerator.xsa'
    if not xsa.exists():
...
def step5(host):
    ...
    required = [VF / 'mlp_accelerator.bit', VF / 'mlp_accelerator.hwh']
    ...
    deploy_files = [
        VF   / 'mlp_accelerator.bit',
        VF   / 'mlp_accelerator.hwh',
        PYNQ / 'mlp_inference_pynq.py',
    ...
    print(f'\n  [2/{n}] mlp_inference_pynq.ipynb (BOM-safe) ...')
    scp_notebook(PYNQ / 'mlp_inference_pynq.ipynb', host)
...
def main():
    ...
    t_total = time.time()
    banner(f'MLP FPGA Pipeline - steps {run_nums}')
    print(f'  Root   : {HERE}')
